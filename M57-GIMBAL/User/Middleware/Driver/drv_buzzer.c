...
{
	BUZZER_BSP_RETURN_T ret;
	return ret = (BUZZER_BSP_RETURN_T)ctx->set_switch(ctx->peripheral_handle, ctx->channel, state);
}

BUZZER_BSP_RETURN_T buzzerBsp_setTimer(buzzer_ctx_t *ctx, uint16_t arr)
{
	BUZZER_BSP_RETURN_T ret;
	return ret = (BUZZER_BSP_RETURN_T)ctx->set_freq(ctx->peripheral_handle, ctx->channel, arr);
}

BUZZER_BSP_RETURN_T buzzzerBsp_getTick(buzzer_ctx_t *ctx, uint32_t* tick_ptr)
{
	BUZZER_BSP_RETURN_T ret;
	return ret = (BUZZER_BSP_RETURN_T)ctx->get_tick(tick_ptr);
}

BUZZER_BSP_RETURN_T buzzerBsp_checkTickTolerance(uint32_t target, uint32_t measure, uint32_t tolerance)
{
	BUZZER_BSP_RETURN_T difference = (BUZZER_BSP_RETURN_T)(target - measure);
