...
// CRC8校验码
static const unsigned char crc_8_table[256] =
    {
...
/* Function prototypes -------------------------------------------------------*/
// 无敌状态检测(否：0，是：1)
void Class_Referee::Detect_invincibility_status(uint8_t *_status)
{
    static Robot_Status_t item[6];
    if (Robot_Status.HP_Max == 0)
        return;

    if (Robot_Status.Robot_ID == Referee_Data_Robots_ID_RED_SENTRY_7)
    {
        item[0].HP = Game_Robot_HP.Blue_Hero_1;
        item[1].HP = Game_Robot_HP.Blue_Engineer_2;
        item[2].HP = Game_Robot_HP.Blue_Infantry_3;
        item[3].HP = Game_Robot_HP.Blue_Infantry_4;
        item[4].HP = Game_Robot_HP.Blue_Infantry_5;
        item[5].HP = Game_Robot_HP.Blue_Sentry_7;
    }
    else if (Robot_Status.Robot_ID == Referee_Data_Robots_ID_BLUE_SENTRY_7)
    {
        item[0].HP = Game_Robot_HP.Red_Hero_1;
        item[1].HP = Game_Robot_HP.Red_Engineer_2;
        item[2].HP = Game_Robot_HP.Red_Infantry_3;
        item[3].HP = Game_Robot_HP.Red_Infantry_4;
        item[4].HP = Game_Robot_HP.Red_Infantry_5;
        item[5].HP = Game_Robot_HP.Red_Sentry_7;
    }
    ...
    if ((Robot_Status.Robot_ID == Referee_Data_Robots_ID_RED_SENTRY_7 && Game_Robot_HP.Blue_Outpost_11 != 0) || (Robot_Status.Robot_ID == Referee_Data_Robots_ID_BLUE_SENTRY_7 && Game_Robot_HP.Blue_Outpost_11 != 0))
    {
        *_status |= 0x01 << 5;
    }
}
 ...
 * @return uint8_t 校验码
 */
uint8_t Class_Referee::CRC_8(uint8_t *Message, uint32_t Length)
{
    uint8_t index;
    uint8_t check = 0xff;
 ...
 * @return uint8_t 校验码
 */
uint16_t Class_Referee::CRC_16(uint8_t *Message, uint32_t Length)
{
    uint8_t index;
...
void Class_Referee::CAN_RxCpltCallback(uint8_t *Rx_Data)
{
    ...
    // game_stage = Rx_Data[0];
    // Game_Status.Remaining_Time = (uint16_t) Rx_Data[1] << 8 | Rx_Data[2];
    // Event_Referee_Warning.Level = (Enum_Referee_Data_Event_Referee_Warning_Level)Rx_Data[3];
    // Robot_Status.HP = (uint16_t) Rx_Data[4] << 8 | Rx_Data[5];
    // is_under_attack = Rx_Data[6];
